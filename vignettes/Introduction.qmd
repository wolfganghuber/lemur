...
As is, the data separate by the known covariates `patient_id` and `condition`.
...
```{r}
#| label: fig-umap_de
#| fig-cap: "Differential expression (log fold changes) of GAP43 superimposed on the same UMAP plot as in @fig-lemur_umap"
#| fig-width: 6
#| fig-height: 4
#| out-width: "80%"
sel_gene <- "ENSG00000172020"   # Gene symbol is GAP43
...
tibble(umap = umap) |>
  ...
  ggplot(aes(x = umap[,1], y = umap[,2])) +
    ...
    labs(title = "Differential expression on UMAP plot")  + coord_fixed()
```
...
```{r}
#| label: fig-tumor_cell_annotation
#| paged.print: false
#| fig-cap: ""
#| fig-width: 6
#| fig-height: 4
...
ggplot(tumor_label_df, aes(x = chr10_total_expr, y = chr7_total_expr)) +
    ...
    labs(title = "Simple gating strategy to find tumor cells")  + coord_fixed()
